...
function initStudentPickerApp() {
    // 检查Vue是否可用
    if (typeof Vue === 'undefined') {
        console.log('Vue尚未加载，1秒后重试...');
        setTimeout(initStudentPickerApp, 1000);
        return;
    }
    
    console.log('Vue已加载，开始初始化学号抽取系统...');
...
// 页面加载完成后初始化
document.addEventListener('DOMContentLoaded', initStudentPickerApp);

// 如果页面已经加载完成，立即初始化
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initStudentPickerApp);
} else {
    initStudentPickerApp();
}
